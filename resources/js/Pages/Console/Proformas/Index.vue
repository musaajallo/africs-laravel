        ...
        <div class="panel-page">
            ...
            <div class="panel-toolbar">
                ...
                <select v-model="status" class="field-input" style="max-width: 11rem">
                    <option value="">Open proformas</option>
            ...
            <PanelTable
                :columns="['Number', 'Client', 'Status', { label: 'Total', align: 'right' }, 'Issued', 'Valid until', { label: 'Actions', align: 'right' }]"
                :rows="proformas.data"
                empty="No proformas match your filter."
            >
                <template #row="{ row }">
                    <td>
                        <Link :href="route('console.proformas.show', row.id)" class="panel-link" style="padding: 0">
                            {{ row.number }}
                        </Link>
                    </td>
                    <td>{{ row.client }}</td>
                    ...
                    <td class="panel-cell-muted">{{ row.valid_until || '—' }}</td>
                    <td class="panel-row-actions">
                        <Link :href="route('console.proformas.show', row.id)" class="panel-link">View</Link>
                        <Link
                            v-if="can('proformas.manage') && row.status === 'draft'"
