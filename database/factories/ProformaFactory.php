<?php

namespace Database\Factories;

use App\Models\Client;
use App\Models\Proforma;
use Illuminate\Database\Eloquent\Factories\Factory;

/**
 * @extends Factory<Proforma>
 */
class ProformaFactory extends Factory
{
    protected $model = Proforma::class;

    public function definition(): array
    {
        return [
            'number' => 'PRO-'.fake()->unique()->numberBetween(1000, 9999999),
            'client_id' => Client::factory(),
            'status' => 'draft',
            'currency' => 'GMD',
            'fx_rate' => 1,
            'issue_date' => now()->toDateString(),
            'valid_until' => now()->addDays(30)->toDateString(),
            'tax_label' => 'VAT',
            'tax_rate' => 0,
        ];
    }

    public function status(string $status): static
    {
        return $this->state(fn () => ['status' => $status]);
    }

    /** Attach a line and recalculate totals after creating. */
    public function withLine(string $description = 'Consulting', string $qty = '1', string $unitPrice = '1000'): static
    {
        return $this->afterCreating(function (Proforma $proforma) use ($description, $qty, $unitPrice) {
            $proforma->lines()->create([
                'description' => $description,
                'quantity' => $qty,
                'unit_price' => $unitPrice,
                'position' => $proforma->lines()->count(),
            ]);
            $proforma->load('lines');
            $proforma->saveWithTotals();
        });
    }
}
